...
from geo_features.geom.point import PointEnum, Point, Point2, PointZ, PointM, PointZM
...
fn test_constructors():
    ...
    _ = Point[2, DType.int32, PointEnum.Point]()
    _ = Point[2, DType.float64, PointEnum.PointM]()
    _ = Point[2, DType.float64, PointEnum.PointZ]()
    _ = Point[2, DType.float64, PointEnum.PointZM]()
    _ = Point[4, DType.float64, PointEnum.PointZM]()

    _ = Point[2, DType.int32](lon, lat)
    _ = Point[2, DType.float64](lon, lat)
    _ = Point[4, DType.float64, PointEnum.PointZM](lon, lat)

    _ = Point[dtype = DType.float16, simd_dims=4, T=PointEnum.PointZM](
...
fn test_from_json() raises:
    ...
    let pt1 = Point2.from_json(json_dict)
    print(pt1.__repr__())
...
fn test_from_wkt() raises:
    ...
    try:
        ...
        let point_2d_f32 = Point[2, DType.float32].from_wkt(wkt)
        test.assert_true(
            point_2d_f32.__repr__()
            == "Point[2, float32](-108.68000000000001, 38.973999999999997)",
            "from_wkt()",
        )
    except:
        raise Error(
